 ...
 * 
 * Modified by Jérémy Laumon
 * 	- added "unique" option
 * 	- corrected default timeout bug
 *   
 * $Id$
 */
 ...
 * --- NEW ---
 * 
 * // play a sound from the url, on a track, does not play another sound on this track until timeout
 * $.sound.play(url, {
 *   track: "track1",
 *   unique: true
 * });
...
$.sound = {
	...
	play: function(url, options){
		...
		setTimeout(function() {
			element.remove();
			if (settings.track) {
				$.sound.tracks[settings.track] = null;
			}
		}, settings.timeout)
